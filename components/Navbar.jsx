...
const Navbar = () => {
    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full bg-white z-50 text-black pr-6 xl:pr:0 transition-all duration-300 ${scrolled ? 'shadow-md' : ''}`}
        ...
        >
                    ...
                    <div className="hidden xl:flex items-center justify-end space-x-1 md:space-x-2 lg:space-x-4 xl:space-x-6 flex-1">
                        {navItems.map((item, index) => (
                            <div key={item.id} className="relative group">
                                <Link
                                    href={item.href}
                                    className={`font-medium text-xs md:text-sm lg:text-base whitespace-nowrap hover:text-purple-500 transition-colors duration-200 flex items-center py-2 px-1 md:px-2 ${item.active ? 'text-purple-500' : 'text-gray-800'}`}
                                ...
                                {item.hasDropdown && activeDropdown === index && (
                                    <div className="fixed left-0 w-full bg-white border-t border-gray-100 shadow-lg z-20"
                                        ...
                                        <div className="container mx-auto py-4 md:py-6 lg:py-8 px-4">
                                            ...
                                            <p className="text-sm text-gray-600">Subcategories for {item.name} would appear here</p>
                                        </div>
                                    </div>
                                )}
                            </div>
            ...
            {isOpen && (
                <div className="xl:hidden bg-white border-t border-gray-200 overflow-hidden max-h-screen overflow-y-auto">
                                        ...
                                        <div>
                                            ...
                                            {activeDropdown === index && (
                                                <div className="pl-4 mt-2 border-l-2 border-purple-500">
                                                    {/* Placeholder for dropdown content */}
                                                    <p className="text-sm text-gray-600 py-2">Subcategories for {item.name} would appear here</p>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
